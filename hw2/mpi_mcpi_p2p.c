...
//somewhere here i am getting error
double parallel_pi(int n, int comm_sz, int my_rank) {
    int local_count=0, count=0;
    int source;
    /*** TODO 2: add code to determine the number of points that are handled by
    each process ***/
    ...
    //i think this is where we do the actual parallizing 
    if(my_rank != 0){
        //we will send our local_count to process 0
...
int main(int argc, char *argv[]) {
    ...
    if (my_rank == 0) {
        /* Check command line arguments for correctness and read the parameters
        from argv */
        if (argc != 2) {
